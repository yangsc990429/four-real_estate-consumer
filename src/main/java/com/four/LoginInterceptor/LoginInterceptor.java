      ...
      System.err.println(url + "我的权限" + path);
           ...
           System.err.println(user.getUserid() + "aa" + mokid + "31313" + request.getSession().getAttribute("quanxian"));
            ...
            for (int i = 0; i < list.size(); i++) {
                qxnames=list.get(i).get("qxname").toString().split(",");
                mkid= (Integer) list.get(i).get("mkid");
            }
            for (int j = 0; j < qxnames.length; j++) {
                if (!qxnames[j].equals("")&&qxnames[j].equals("0")){
                    count=1;
                    break;
                }else{
                    if (url.equals(qxnames[j]) && mkid== mokid) {
