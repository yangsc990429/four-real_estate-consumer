var timer = null;
GLOBAL.Namespace('View.CommunityCycle', function () {
    var _this = this,
    ...
    _this.Init = function () {
        ...
        currentUrlCycle = $('#currentUrlCycle').val();
        //02 搜索事件
        bindSearchEvent(currentUrlCycle);
        ////03 加载上次搜索条件
        LoadLastSearchTerm(currentUrlCycle);
        ////03 加载上次搜索条件
        //SaveSearchTerm(currentUrlCycle);      
...
function next(reid) {
    ...
    $.ajax({
        type: "GET",
        url: '/Service/CommunityCycle.ashx',
        async: true,
        cache: false,
    ...
    $("#mCSB_3_container").closest('div').css('top', '0px');
...
function ClkPage(params) {
    ...
    $.ajax({
        type: "GET",
        url: '/Service/CommunityCycle.ashx?v=' + new Date().getTime(),
        async: true,
        cache: false,
        data: params + "&Type=getList&flag=1&HT=" + HousingType,
        success: function (result) { 
            $('.map_ico_left').empty();
            var arr = result.split('@');
            //$("#CommunityCount").html("1");
...
function Areashows() {
    ...
    for (var i = 0; i < MtDictInfo_Tbl.length; i++) {
        if (DictInType == 0 || MtDictInfo_Tbl[i].dictId == DictInfoONE_DictID || MtDictInfo_Tbl[i].dictId == RE_ID_RegionID) {
            ...
            var textem = MtDictInfo_Tbl[i].DictHJ + " 个";
