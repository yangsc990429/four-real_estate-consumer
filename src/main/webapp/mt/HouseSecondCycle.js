var timer = null;
GLOBAL.Namespace('View.HouseSecondCycle', function () {
    var _this = this,
...
function next(reid) {
    ...
    $.ajax({
        type: "GET",
        url: '/Service/HouseSecondCycle.ashx',
        async: true,
        cache: false,
        ...
        success: function (result) {
            ...
            re = /满五唯一/g; // 创建正则表达式模式。
            $(".map_ico_left").html(arr[1].replace(re, "满五"));
            $(".paging").html(arr[2]);
    ...
    $("#mCSB_6_dragger_vertical").closest('div').css('top', '0px');
...
function clkPaixu(aa,params,re)
{
    ...
    if (ys == '12')
    {
        $('.paixu_zj').html("<a onclick=ga('send', 'event', 'paixu', 'zongjia', 'yougaodaodi'); href=javascript:clkPaixu(this,'OR=11','" + RE + "') class='bg'>总价<i class='mai-ico'></i></a>");
        $('.paixu_dj').find('a').removeClass('bg');
        $('.paixu_mj').find('a').removeClass('bg');
        $('.paixu_dj').find('i').removeClass('up');
        $('.paixu_mj').find('i').removeClass('up');
    }
    if (ys == '11') {
        $('.paixu_zj').html("<a onclick=ga('send', 'event', 'paixu', 'zongjia', 'yougaodaodi'); href=javascript:clkPaixu(this,'OR=12','" + RE + "') class='bg'>总价<i class='mai-ico up'></i></a>");
        $('.paixu_dj').find('a').removeClass('bg');
        $('.paixu_mj').find('a').removeClass('bg');
        $('.paixu_dj').find('i').removeClass('up');
        $('.paixu_mj').find('i').removeClass('up');
    ...
    if (ys == '22') {
        $('.paixu_dj').html("<a onclick=ga('send', 'event', 'paixu', 'danjia', 'yougaodaodi'); href=javascript:clkPaixu(this,'OR=21','" + RE + "') class='bg'>单价<i class='mai-ico'></i></a>");
        $('.paixu_zj').find('a').removeClass('bg');
        $('.paixu_mj').find('a').removeClass('bg');
        $('.paixu_zj').find('i').removeClass('up');
        $('.paixu_mj').find('i').removeClass('up');
    }
    if (ys == '21') {
        $('.paixu_dj').html("<a onclick=ga('send', 'event', 'paixu', 'danjia', 'yougaodaodi'); href=javascript:clkPaixu(this,'OR=22','" + RE + "') class='bg'>单价<i class='mai-ico up'></i></a>");
        $('.paixu_zj').find('a').removeClass('bg');
        $('.paixu_mj').find('a').removeClass('bg');
        $('.paixu_zj').find('i').removeClass('up');
        $('.paixu_mj').find('i').removeClass('up');
    ...
    if (ys == '32') {
        $('.paixu_mj').html("<a onclick=ga('send', 'event', 'paixu', 'mianji', 'yougaodaodi'); href=javascript:clkPaixu(this,'OR=31','" + RE + "') class='bg'>面积<i class='mai-ico'></i></a>");
        $('.paixu_dj').find('a').removeClass('bg');
        $('.paixu_zj').find('a').removeClass('bg');
        $('.paixu_dj').find('i').removeClass('up');
        $('.paixu_zj').find('i').removeClass('up');
    ...
    if (ys == '31') {
        $('.paixu_mj').html("<a onclick=ga('send', 'event', 'paixu', 'mianji', 'yougaodaodi'); href=javascript:clkPaixu(this,'OR=32','" + RE + "') class='bg'>面积<i class='mai-ico up'></i></a>");
        $('.paixu_dj').find('a').removeClass('bg');
        $('.paixu_zj').find('a').removeClass('bg');
        $('.paixu_dj').find('i').removeClass('up');
        $('.paixu_zj').find('i').removeClass('up');
    ...
    $.ajax({
        type: "GET",
        url: '/Service/HouseSecondCycle.ashx?v=' + new Date().getTime(),
        async: true,
        cache: false,
        data: params + "&Type=getList&RE=" + RE + context,
        success: function (result) {
            $('.map_ico_left').empty();
            var arr = result.split('@');
            $("#EsfCount").html(arr[0]);
    ...
    $("#mCSB_6_container").closest('div').css('top', '0px');
    $("#mCSB_6_dragger_vertical").css('top', '0px');
}
    ...
    for (var i = 0; i < GetReInfo_View.length; i++) {
                    ...
                    VillageLabel = new BMap.Label(" <a href='javascript:;'    onclick =\"next('" + GetReInfo_View[i].CommunityID + "')\"  onmouseenter =\"addZindex(this)\"  > <div class='single_price2'  id='pShowB'   ><p><span class='single_price_num'><label>" + GetReInfo_View[i].AveragePrice + "</label>万</span><span class='single_name_num'  id='pShowA' >  " + GetReInfo_View[i].CommunityName + "<label> " + GetReInfo_View[i].HSCOUNTIsRealSourceOn + "</label>套</span></p><em></em></div> </a>",
       {
           offset: new BMap.Size(0, 0), position: point
                    ...
                    VillageLabel = new BMap.Label(" <a href='javascript:;'    onclick =\"next('" + GetReInfo_View[i].CommunityID + "')\"  onmouseenter =\"addZindex(this)\"  > <div class='single_price2'  id='pShowB'   ><p><span class='single_price_num'><label>" + GetReInfo_View[i].AveragePrice + "</label>万</span><span class='single_name_num'  id='pShowA' >  " + GetReInfo_View[i].CommunityName + "<label> " + GetReInfo_View[i].HZCountIsRealSourceOn + "</label>套</span></p><em></em></div> </a>",
{
    offset: new BMap.Size(0, 0), position: point
});                           //label的偏移量，为了让label的中心显示在点上        //label的位置
