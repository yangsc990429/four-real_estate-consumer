...
//Date:2014。12.03
//*/
GLOBAL.Namespace('View.HouseSecondDetail', function () {
    var _this = this,
            ...
            GLOBAL.DataStore.DataHandle.postData({
                ...
                success: function (data) {
                    ...
                    GLOBAL.DataStore.DataHandle.postData({
                        url: '/Service/Main.ashx',
                        handler: 'data',
                        parame: '{fucid:"reportHouseCommentByMemberID",data:"' + _houseCommentID + ',' + HouseID + ',' + MemberID + ','+title+',' + reportInfo + ',1,' + data.NiceName + ',' + data.Mobile + '"} ',
        ...
        if (MemberID != null && MemberID.length > 0 && HouseID != null && HouseID.length > 0) {
            ...
            GLOBAL.DataStore.DataHandle.postData({
                ...
                success: function (data) {
                    ...
                    var title = "";
                    if (reportInfo.length > 500) {
                        title = reportInfo.substring(0, 500);
                    }
                    else {
                        title = reportInfo;
                    }
                    ...
                    GLOBAL.DataStore.DataHandle.postData({
                        url: '/Service/Main.ashx',
                        handler: 'data',
                        parame: '{fucid:"reportHouseByMemberID",data:"' + HouseID + ',' + MemberID + ',' + title + ',' + reportInfo + ',1,' + data.NiceName + ',' + data.Mobile + '"} ',
                        success: function (result) {
                            if (result.status == '200') {
                                alert('举报成功！');
                                $('.reveal-modal-bg').hide();
...
function IsCollectHouseByMemberID() {
    var MemberID = GLOBAL.Cookie('MemberID');
    if (MemberID != null && MemberID.length > 0 && HouseCode.length > 0) {
        GLOBAL.DataStore.DataHandle.getData({
            url: '/Service/Main.ashx',
            handler: 'data',
            parame: '{fucid:"IsCollectHouseByMemberID",data:"' + MemberID + ',' + HouseCode + '"} ',
            success: function (result) {
                if (result[0].RESULT == '1') {
                    $('#CollectHouse').attr('class', 'btn_qx');

                } else if (result[0].RESULT == '0') {
                    $('#CollectHouse').attr('class', 'btn_gz');
                }
            }
        });
    }
...
function CollectHouse() {
    ...
    $('#CollectHouse').attr('class') == 'btn_gz' ? $('#CollectHouse').attr('class', 'btn_qx') : $('#CollectHouse').attr('class', 'btn_gz');
    if (MemberID != null && MemberID.length > 0 && HouseCode.length > 0) {
        GLOBAL.DataStore.DataHandle.postData({
            url: GLOBAL.PageBase.ServiceURL,
            handler: 'SaveCollectInfo_perm',
            parame: '{fucid:"RequestHandler",data:"1,' + MemberID + ',' + HouseCode + '"} ',
            success: function (result) {
                //
            }
        });
...
function LoadMatching() {
    if (SideMatching != undefined && SideMatching!=null){
        for (var i = 0; i < SideMatching.length; i++) {
            var MatchingLi = "<li onclick=\"ga('send', 'event', 'shenghuoquan', 'tab_qiehuan', '" + SideMatching[i].Matching + "');\" index=\"" + i + "\" val=\"" + SideMatching[i].Matching + "\"><i class=\"mai-ico\"></i>" + SideMatching[i].Matching + "(" + SideMatching[i].PointList.length + ")</li>";

            if (i == 0) {
                MatchingLi = "<li onclick=\"ga('send', 'event', 'shenghuoquan', 'tab_qiehuan', '" + SideMatching[i].Matching + "');\" class=\"hit\" val=\"" + SideMatching[i].Matching + "\" index=\"" + i + "\"><i class=\"mai-ico\"></i>" + SideMatching[i].Matching + "(" + SideMatching[i].PointList.length + ")</li>";
                LoadPoint(0);
            }
...
function LoadPoint(i) {
    ...
    for (var p = 0; p < PointClass.length; p++) {
        ...
        var NewList = "<div class=\"fk_xinxi clearfix\"><dl class=\"mai-ico\">" + parseInt(p + 1) + "</dl><ul><span style='cursor:pointer;' onclick=\"disPoint(" + p + ");ga('send', 'event', 'shenghuoquan', '" + SideMatching[i].Matching + "', '" + PointClass[p].Name + "');\">" + PointClass[p].Name + "</span><li class=\"site\">地址：</li><li class=\"line\">" + PointClass[p].Address + "</li></ul></div>";
...
function phoneLogClick(phonenum, Brokerid, houseno, obj, type,tjType) {
    var MemberID = GLOBAL.Cookie('MemberID');
    var logobj = [
         URLS= '/VIEW/HouseSecond/HouseSecondDetail.html',
         ...
         DEVICENAME='',
         PAGENAME = escape('二手房房源详情页'),
         HOUSENO = houseno,
         AREATYPE = 'bj',//bj-北京，fz-福州，xm-厦门
         PAGETYPE = '110200',//页面类型 《900000-其他，100000-首页，110000-二手房，110100-二手房列表，110200-二手房详情，120000-租房，120100-租房列表，120200-租房详情，130000-小区，130100-小区列表，130200-小区详情，140000-房产顾问，140100-房产顾问列表，140200-房产顾问详情，150000-会员中心，150100-会员中心/我关注的房产顾问》
         ACTIONTYPE = tjType == 1 ? (type == 1 ? '150101' : (type == 2 ? '150102' : '150103')): (type == 1 ? '110101' : (type == 2 ? '110102' : '110103'))//行为类型 《900000-其他,100000-浏览，110000-点击查看电话，110101-二手房详情/首屏3D，110102-二手房详情/房顾说，110103-二手房详情/小区历史成交，110201-租房详情/首屏3D，110202-租房详情/房顾说，110301-小区详情/首屏3D，110401-房产顾问列表/首屏3D，110501-房产顾问详情/首屏3D，110601-会员中心/我关注的房产顾问，120000-检索，130000-下载,140000-对比》
    ...
    GLOBAL.DataStore.DataHandle.getData({
        ...
        success: function (result) {
            if (result.status == '200') {
                ...
                if (type == 2 && tjType == 2 || type == 3 && tjType == 2) {
