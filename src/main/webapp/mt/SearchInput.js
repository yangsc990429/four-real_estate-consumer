...
function bindSearchEvent(getUrl) {
    ...
    $('#ico-search').bind('input propertychange', function (event) {
        ...
        if (searchContent == "请输入房源所在区域或小区名称" || searchContent == "请输入区域或小区名称") {
            searchContent = "";
        }
        //alert(getUrl);
        if (searchContent.length > 0) {
            ...
            setTimeout(function () {
                $.ajax({
                    ...
                    data: { OperType: "SearchHouse", curUrl: getUrl, RS: searchContent },
                    dataType: "html",
    ...
    $('#ico-search').focus(function () {
        ...
        //var cookieName = "hisSearchTermCookie";
        var cookieName;
        if (getUrl.indexOf("HouseSecondList") > -1) {
            cookieName = "righthisSearchHouseSecondCookie";
        }
        if (getUrl.indexOf("HouseRentList") > -1) {
            cookieName = "righthisSearchHouseRentCookie";
        }
        if (getUrl.indexOf("EliteHousingList") > -1) {
            cookieName = "righthisSearchEliteHousingListCookie";
        }
        if (getUrl.indexOf("CommunityList") > -1) {
            cookieName = "righthisSearchCommunityCookie";
        }
        if (getUrl.indexOf("HouseSecondSubwayList") > -1) {
            cookieName = "righthisSearchHouseSecondCookie";
        }
        if (getUrl.indexOf("HouseRentSubwayList") > -1) {
            cookieName = "righthisSearchHouseRentCookie";
        }
        if (getUrl.indexOf("EliteHousingSubWayList") > -1) {
            cookieName = "righthisSearchEliteHousingSubWayListCookie";
        }
        if (getUrl.indexOf("CommunitySubwayList") > -1) {
            cookieName = "righthisSearchCommunityCookie";
        }
        //debugger;
        //$('#lishijilu').val("历史记录");
        ...
        if (json != null && json.length > 0) {
            ...
            content = "<li style=\"height:35px;line-height:34px;padding:0 4px;background:#fff;font-size:14px \">搜索历史    <a target=\"_blank\" style=\"display:inline-block;float:right;height:35px;line-height:34px;width:15%;cursor:pointer;background:none; \"  onclick=\"dataProxy.removeItem('" + cookieName + "')\" >清除历史记录 </a></li>";
    ...
    $('#btSearch').live('click', function () {
        ...
        //if (con != "请输入房源所在区域或小区名称" && con != "") {
        con = con == "请输入房源所在区域或小区名称" ? "" : con;
        con = encodeURIComponent(con);
        ga('send', 'pageview', getUrl + '/RS' + con);
        if (con == null || con == undefined || con == '') {
            if (window.location.pathname.toLocaleLowerCase().indexOf('index.html') > 0 || window.location.pathname.toLocaleLowerCase().indexOf('index2.html') > 0)//首页跳转二手房
            {
                getUrl = "/View/Housesecond/HousesecondList.html";
            }
            else {
                getUrl = getUrl.substr(0, getUrl.indexOf("?") + 1) + "PG=1&";
            }
        }
        else {
            getUrl = getUrl.replace(/PG=\d{0,7}&/gi, "PG=1&");
        }
        $('#ico-search').val('');
        //请求查询
        ...
        /*去掉上次搜索框内的内容，就是RS的值start*/
        //var getUrlunit = getUrl.split('&');
        //getUrl = '';
        //for (var i = 0; i < getUrlunit.length; i++) {
        //    if (getUrlunit[i].indexOf('RS=') == -1) {
        //        if (i < getUrlunit.length - 1) {
        //            getUrl += getUrlunit[i] + '&';
        //        }
        //        else {
        //            getUrl += getUrlunit[i] ;
        ...
        //    }
        //}
        /*去掉上次搜索框内的内容，就是RS的值end*/
        //console.log(getUrl);
        //debugger;
        //请求查询
        //window.location.href = GetPageUrl({ url: getUrl, param1: "rs=" + con });
...
function LoadLastSearchTerm(getUrl) {
    ...
    if (getUrl.indexOf("EliteHousingList") > -1) {
        cookieName = "hisSearchEliteHousingListCookie";
    }
    if (getUrl.indexOf("CommunityList") > -1) {
    ...
    if (getUrl.indexOf("EliteHousingSubWayList") > -1) {
        cookieName = "hisSearchEliteHousingSubWayListCookie";
    }
    if (getUrl.indexOf("CommunitySubwayList") > -1) {
        cookieName = "hisSearchCommunityCookie";
    }
...
function createHistoryCookie(getUrl) {
    ...
    var cookieName;
    if (urlcookie.indexOf("HouseSecondList") > -1) {
        cookieName = "righthisSearchHouseSecondCookie";
    }
    if (urlcookie.indexOf("HouseRentList") > -1) {
        cookieName = "righthisSearchHouseRentCookie";
    }
    if (urlcookie.indexOf("EliteHousingList") > -1) {
        cookieName = "righthisSearchEliteHousingListCookie";
    }
    if (urlcookie.indexOf("CommunityList") > -1) {
        cookieName = "righthisSearchCommunityCookie";
    }
    if (urlcookie.indexOf("HouseSecondSubwayList") > -1) {
        cookieName = "righthisSearchHouseSecondCookie";
    }
    if (urlcookie.indexOf("HouseRentSubwayList") > -1) {
        cookieName = "righthisSearchHouseRentCookie";
    }
    if (urlcookie.indexOf("EliteHousingSubWayList") > -1) {
        cookieName = "righthisSearchEliteHousingSubWayListCookie";
    }
    if (urlcookie.indexOf("CommunitySubwayList") > -1) {
        cookieName = "righthisSearchCommunityCookie";
    }
