...
function bindSearchEvent(getUrl) {
        ...
        if ($("#SearchTips").css("display") == "block") {
            ...
            if (event.keyCode == 38) {
                if (upDownClickNum < 1) {
                    ...
                    $("#SearchTips .searchBackgroud ").removeClass("searchBackgroud").css({ "background": "" }).parent().css({ "background": "" }).prev().children().addClass("searchBackgroud").css({ "background": "#f0f0f0" }).parent().css({ "background": "#f0f0f0" });
                }
                stopDefault(event);
                ...
                if (upDownClickNum < 1) {
                    ...
                    $("#SearchTips .searchBackgroud").removeClass("searchBackgroud").css({ "background": "" }).parent().css({ "background": "" }).next().children().addClass("searchBackgroud").css({ "background": "#f0f0f0" }).parent().css({ "background": "#f0f0f0" });
                }
                stopDefault(event);
            }
        }
    ...
    $('#ico-search').bind('input propertychange', function (event) {
        ...
        if (searchContent == "请输入房源所在区域或小区名称") {
            searchContent = "";
        }
        if (searchContent.length > 0) {
            ...
            setTimeout(function () {
                $.ajax({
                    ...
                    data: { OperType: "SearchHouseCycle", curUrl: getUrl, RS: searchContent },
                    dataType: "html",
                    success: function (data) {
                        SearchResult = decodeURIComponent(data);                       
                        SearchResult = SearchResult.replace(/r=\d&/gi, "")
    ...
    $('#ico-search').focus(function () {
        ...
        if (getUrl.indexOf("HouseSecondCycle") > -1) {
            cookieName = "righthisSearchHouseSecondCookie";
        }
        if (getUrl.indexOf("EliteHousingCycle") > -1) {
            cookieName = "righthisSearchEliteHousingCycleCookie";
        }
        if (getUrl.indexOf("HouseRentCycle") > -1) {
            cookieName = "righthisSearchHouseRentCookie";
        }
        if (getUrl.indexOf("CommunityList") > -1) {
            cookieName = "righthisSearchCommunityCookie";
        }
        if (getUrl.indexOf("CommunityCycle") > -1) {
            cookieName = "righthisSearchCommunityCookie";
        }
        ...
        if (json != null && json.length > 0) {
            ...
            content = "<li style=\"height:35px;line-height:34px;padding:0 4px;background:#fff \">搜索历史    <a target=\"_blank\" style=\"display:inline-block;float:right;height:35px;line-height:34px;width:15%;cursor:pointer;background:none; \"  onclick=\"dataProxy.removeItem('" + cookieName + "')\" >清除历史记录 </a></li>";
    ...
    $('#btSearch').live('click', function () {
        ...
        //if (con != "输入房源所在区域及小区名称" && con != "") {
        con = con == "请输入房源所在区域或小区名称" ? "" : con;
        con = encodeURIComponent(con);
        ga('send', 'pageview', getUrl + '/RS' + con);
        resultUrl=GetPageUrl({ url: getUrl, param1: "rs=" + con });
        if (resultUrl.indexOf("/RE")) {
            var reg = /\/RE[0-9a-zA-Z]{32}/;
...
function LoadLastSearchTerm(getUrl) {
    var cookieName;
    //if (getCookie("_AreaValue") == "3") {
    //    cookieName = "FZ_hisSearchTerm";

    //}
    //else if (getCookie("_AreaValue") == "4") {
    //    cookieName = "XM_hisSearchTerm";
    //}
    //cookieName = "hisSearchTerm";
    //var json = eval(decodeURIComponent(GLOBAL.Cookie(cookieName)));
    if (getUrl.indexOf("HouseSecondCycle") > -1) {
    ...
    if (getUrl.indexOf("EliteHousingCycle") > -1) {
        cookieName = "hisSearchEliteHousingCycleCookie";
    }
    if (getUrl.indexOf("HouseRentCycle") > -1) {
    ...
    //如果有缓存，则取出遍历
    if (json != null && json.length > 0) {
        $('#lastSearchTerm').html("[上次找寻条件:&nbsp;<a target=\"_blank\" href=" + json[0].url + " onclick=\"ga('send', 'event', 'zhanneisousuo', 'jiansuotiaojian', 'shangcizhaoxuntiaojian');\">" + json[0].content + "</a>]");
    } else {
        $('#lastSearchTerm').html("");
    }
    //创建Cookie
...
function createHistoryCookie(getUrl) {
    ...
    if (urlcookie.indexOf("HouseSecondCycle") > -1) {
        cookieName = "righthisSearchHouseSecondCookie";
    }
    if (urlcookie.indexOf("EliteHousingCycle") > -1) {
        cookieName = "righthisSearchEliteHousingCycleCookie";
    }
    if (urlcookie.indexOf("HouseRentCycle") > -1) {
        cookieName = "righthisSearchHouseRentCookie";
    }
    if (urlcookie.indexOf("CommunityList") > -1) {
        cookieName = "righthisSearchCommunityCookie";
    }
    if (urlcookie.indexOf("CommunityCycle") > -1) {
        cookieName = "righthisSearchCommunityCookie";
    }
...
function getCycleSearchHouseTerm() {
    ...
    var Result = "";
    if (SearchPrice != "" && SearchPrice != "售价" && SearchPrice != "租金" && SearchPrice != undefined) {
        Result += SearchPrice + " | ";
    }
