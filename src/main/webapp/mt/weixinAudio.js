(function () {
    $.fn.weixinAudio = function(options) {
        ...
        function Plugin($context) {
            ...
            this.$next = $context.closest('li').next().find('.weixinAudio');
            this.$next.$audio_area = $context.closest('li').next().find('#audio_area');
            //执行初始化
            this.init();
        }
        Plugin.prototype = {
			...
			audioWidth:function(rangeMin,rangeMax,string){
			    var self = this;
			    //console.log(parseInt(string) / .6 / 100 * (rangeMax - rangeMin) + rangeMin);
                var w = parseInt(string)/.6/100*(rangeMax-rangeMin)+rangeMin;
                self.$context.find('.audio_wrp').css('width',w);
			},
            //正在播放，播放完成
            run: function() {
                var self = this;
                if (self.Audio.ended) {
                    ...
                    self.pause();
                    if(self.$next.hasClass('noreaded')){
                        self.$next.$audio_area.trigger('click');
                        self.$next.removeClass('noreaded');
					}
            ...
            getAudioSeconds: function(string) {
                ...
                self.audioWidth(20,200,string);
                string = String(Math.round(string))+"''";
                return string;
            },
            //获取时间分
            ...
            updateTotalTime: function() {
                var self = this,
                    ...
                    audioTime = seconds;
                if (audioTime.indexOf('NaN')>-1) { audioTime = ''; }
