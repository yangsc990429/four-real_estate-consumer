...
<body>
<button type="button" class="btn btn-danger" onclick="deleteyChuZu()" >批量删除</button>
<table id="ChuZuys"></table>
...
<script>
    ...
    function deleteyChuZu() {
        //获取所有被选中的记录
        var rows = $("#ChuZuys").bootstrapTable('getSelections');
        if (rows.length== 0) {
            alert("请先选择要删除的记录!");
            return;
        }
        ...
        if(confirm("确认删除吗?")){
            $.ajax({
                url:"<%=request.getContextPath()%>/gby/deleteyChuZu",
                type:"post",
                data:{"id":ids},
                dataType:"text",
                success:function (delFlag){
                    $("#ChuZuys").bootstrapTable('refresh');
