...
<body>
<button type="button" class="btn btn-danger" onclick="deleteyuyue()" >批量删除</button>
<table id="suoyouys"></table>
...
<script>
    var tt="";
    $("#suoyouys").bootstrapTable({
        url:"<%=request.getContextPath()%>/gby/querysuoyou",
        striped: true,//隔行变色
    ...
    function deleteyuyue() {
        //获取所有被选中的记录
        var rows = $("#suoyouys").bootstrapTable('getSelections');
        if (rows.length== 0) {
            alert("请先选择要删除的记录!");
            return;
        }
        ...
        if(confirm("确认删除吗?")){
            $.ajax({
                url:"<%=request.getContextPath()%>/gby/deleteyuyue",
                type:"post",
                data:{"id":ids},
                dataType:"text",
                success:function (delFlag){
                    $("#suoyouys").bootstrapTable('refresh');
