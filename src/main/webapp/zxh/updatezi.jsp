...
<form id="updatezi">
    ...
    <table>
        <tr>
            ...
            <td width="220px" colspan="2">
                <input type="hidden" name="photoimg" id="photo2">
    ...
    <table>
        ...
        <tr>
            <td width="130px" align="right"><font size="4"><strong>审核状态:</strong></font>&nbsp;</td>
            <td width="220px" colspan="2">
                <input type="radio" name="shenhe" value="1">&nbsp;<strong>正常展示</strong>&nbsp;
                <input type="radio" name="shenhe" value="2">&nbsp;<strong>正在审核</strong>&nbsp;
                <input type="radio" name="shenhe" value="3">&nbsp;<strong>审核不通过</strong>
            </td>
            <td width="220px"></td>
            <td width="220px"></td>
        </tr>
        ...
        <tr>
            <td width="150px" colspan="5" align="center">
                <button type="button" class="btn btn-success" onclick="updateZixunId()">保存修改</button>
...
<div class="modal fade" id="myModalasupsucc" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<script type="text/javascript">
    ...
    $.ajax({
        url:"<%=request.getContextPath()%>/zxh/selectQueryId",
        type:"post",
        data:{"id":id},
        dataType:"json",
        success:function (consult){
            $("[name='id']").val(consult.id);
            $("[name='name']").val(consult.name);
            $("[name='zid']").val(consult.zid);
            $("[name='jiacu']").val(consult.jiacu);
            $("[name='color']").val(consult.color);
            $("[name='author']").val(consult.author);
            $("[name='laiyuan']").val(consult.laiyuan);
            $("[name='wangzhi']").val(consult.wangzhi);
            $("[name='photoimg']").val(consult.photoimg);
            $("#imgId").attr("src",consult.photoimg);
            $("[name='description']").val(consult.description);
            $("[name='seozi']").val(consult.seozi);
            $("[name='seodescription']").val(consult.seodescription);
            $("[name='createdate']").val(consult.createdate);
            $("[name='dianlu']").val(consult.dianlu);
            $("[name='fanuhuiyuan']").val(consult.fanuhuiyuan);
            $("[name='toutiao']").val([consult.toutiao]);
            $("[name='shouye']").val([consult.shouye]);
            $("[name='shenhe']").val([consult.shenhe]);
        }
    })

    function fan(){
        var falg = <%=request.getParameter("flag")%>
        if(falg == 1){
            location.href="<%=request.getContextPath()%>/zxh/zixunlist.jsp";
        }else{
            location.href="<%=request.getContextPath()%>/zxh/shenhezixun.jsp";
        }
    }

    function updateZixunId(){
        $.ajax({
            url:"<%=request.getContextPath()%>/zxh/updateZiId",
            type:"post",
            data:$("#updatezi").serialize(),
            dataType:"text",
            success:function (data){
                if(data == "success"){
                    $("#myModalasupsucc").modal({
                        keyboard:false,
                        backdrop:false,
                    })
                }
